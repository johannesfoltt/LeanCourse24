...
structure LindenbaumCatofTheory (T : Theory L) where
  ...
  constTermSubst {α : Type u'} [DecidableEq α] {β : Type v'} [DecidableEq β] (n : ℕ) (φ : Formula L α) (hφ : numVarForm φ = n) (t : α → L.Term β) (ht : ∀ (i : α), numVarTerm (t i) = 0) (hφsub : numVarForm (BoundedFormula.subst φ t) = 0) (ι : Fin n → BoundedFormula.freeVarFinset φ): formToMap 0 (BoundedFormula.subst φ t) hφsub = Pi.lift (fun (i : Fin n) ↦ termToMap 0 (t (ι i)) (ht (ι i))) ≫ formToMap n φ hφ
