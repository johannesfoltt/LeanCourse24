...
theorem neg_neg (a : R) : - -a = a := by
  apply neg_eq_of_add_eq_zero
  rw[← add_zero (-a + a), add_assoc, neg_add_cancel_left]
...
theorem self_sub (a : R) : a - a = 0 := by
  rw[sub_eq_add_neg a a, ← zero_add (a + -a), ← add_assoc, add_neg_cancel_right]
...
theorem mul_one (a : G) : a * 1 = a := by
  rw[← inv_mul_cancel a, ← mul_assoc a a⁻¹, mul_inv_cancel a, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw[← one_mul b⁻¹, ← inv_mul_cancel (a * b), mul_assoc, mul_assoc, mul_assoc,← mul_assoc b b⁻¹ a⁻¹,
  mul_inv_cancel b, one_mul a⁻¹, mul_inv_cancel a, mul_one]
